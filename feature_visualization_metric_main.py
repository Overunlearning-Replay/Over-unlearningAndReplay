...
def kl_divergence(P, Q):
    """计算两个矩阵之间的KL散度，确保没有零值问题"""
    # 将所有的零值替换为很小的正数，防止计算log(0)
...
def feature_visualization_metric(unlearned_model, device):
    ...
    # Q2 = tsne_visualization(full_train_dl, unlearned_model, int(args.forget_class), device, name='unlearned_model', seed=41)
...
def estimate_overlap(Y, labels):
    ...
    grid_points = np.linspace(np.min(Y), np.max(Y), 100)  # 根据数据调整
    ...
    overlap = np.min(densities, axis=0).sum()  # 取最小密度求和作为重叠度量
    return overlap
...
if __name__ == '__main__':
    ...
    # counterpart_train, counterpart_valid = classwise_train[0], classwise_test[0]
    ...
    # print("forget_train_dl", len(forget_train_dl.dataset))
    # print("full_train_dl", len(ConcatDataset((retain_train_dl.dataset, forget_train_dl.dataset))))
